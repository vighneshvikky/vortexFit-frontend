  ...
  onSubmit(): void {
    if (this.verificationForm.valid && this.trainerId) {
      ...
      this.trainerService.updateProfile(this.trainerId, formData).subscribe({
        ...
        error: (error) => {
          this.isLoading = false;
          this.notyService.showError(error?.error?.message || 'Failed to submit verification request');
        }
      });
    } else {
      if (!this.trainerId) {
        this.notyService.showError('Trainer ID not found. Please try logging in again.');
      }
      this.verificationForm.markAllAsTouched();
    }
  }

  get name() { return this.verificationForm.get('name'); }
  get email() { return this.verificationForm.get('email'); }
  get phoneNumber() { return this.verificationForm.get('phoneNumber'); }
  get experience() { return this.verificationForm.get('experience'); }
  get bio() { return this.verificationForm.get('bio'); }
  get specialization() { return this.verificationForm.get('specialization'); }
  get certification() { return this.verificationForm.get('certification'); }
  get idProof() { return this.verificationForm.get('idProof'); }
}
